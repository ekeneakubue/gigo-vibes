...
import { PaymentAccessForm } from "../components/payment-access-form";
import {
...
export default async function PaymentPage({ searchParams }: PaymentPageProps) {
  ...
  const access = await verifyPaymentAccessToken(accessCookie);

  let hasAccess = false;
  if (access?.email) {
    const enrollment = await findEnrollmentByEmail(access.email);
      ...
      {hasAccess ? (
        <div className="relative z-10 mx-auto flex w-full max-w-3xl flex-1 flex-col items-center justify-center px-5 py-16 text-center sm:px-8 sm:py-15">
          ...
          <p className="mt-5 max-w-xl text-base leading-7 text-muted text-pretty sm:text-lg">
            You&apos;re in for life. Your payment email keeps this Telegram
            invite unlocked — come back anytime with the same address if you
            switch devices.
          </p>
